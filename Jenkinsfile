    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-5.9"
          ...
          sh "docker push ${IMAGE}:arm32v7-5.9"
          sh "docker push ${IMAGE}:arm64v8-5.9"
          sh "docker manifest push --purge ${IMAGE}:5.9 || :"
          sh "docker manifest create ${IMAGE}:5.9 ${IMAGE}:amd64-5.9 ${IMAGE}:arm32v7-5.9 ${IMAGE}:arm64v8-5.9"
          sh "docker manifest annotate ${IMAGE}:5.9 ${IMAGE}:arm32v7-5.9 --os linux --arch arm"
