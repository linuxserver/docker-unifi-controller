  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sX GET http://dl-origin.ubnt.com/unifi/debian/dists/unifi-5.6/ubiquiti/binary-amd64/Packages |grep -A 7 -m 1 'Package: unifi' | awk -F ': ' '/Version/{print $2;exit}' | awk -F '-' '{print $1}' ''',
            returnStdout: true).trim()
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:arm32v6-${META_TAG} ${IMAGE}:arm32v6-LTS"
          ...
          sh "docker push ${IMAGE}:arm32v6-LTS"
          sh "docker push ${IMAGE}:arm64v8-LTS"
          sh "docker manifest push --purge ${IMAGE}:LTS || :"
          sh "docker manifest create ${IMAGE}:LTS ${IMAGE}:amd64-LTS ${IMAGE}:arm32v6-LTS ${IMAGE}:arm64v8-LTS"
          sh "docker manifest annotate ${IMAGE}:LTS ${IMAGE}:arm32v6-LTS --os linux --arch arm"
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}' to LTS",\
             "type": "commit",\
